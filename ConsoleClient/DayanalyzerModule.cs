...
namespace ConsoleClient
{
	...
	//		var query = new Query<FeatureUserRow>(@"SELECT ID, NAME FROM FEATURE_USERS WHERE ID = @USER", FeatureUserCreator,
	//			sqlParams);
	...
	//		var query = new Query<FeatureUserRow>(@"SELECT ID, NAME FROM FEATURE_USERS", FeatureUserCreator);

	//		var users = new Dictionary<long, FeatureUserRow>();
	...
	//		var versions =
	//			context.Execute(new Query<FeatureVersionRow>(query, r => new FeatureVersionRow(r.GetInt64(0), r.GetString(1))));
	...
	//	private static FeatureUserRow FeatureUserCreator(IFieldDataReader r)
	//	{
	//		return new FeatureUserRow(r.GetInt64(0), r.GetString(1));
	//	}
	...
	//	public FeatureReport(FeatureUserRow user, FeatureTimes[] slowestFeatures, FeatureTimes[] mostUsedFeatures,
	...
	//	private static FeatureReport ExtractFeatureReport(FeatureUserRow featureUser, FeatureReportSettings settings, List<FeatureTimeEntry> entries, int startIndex, int endIndex)
	//	{
